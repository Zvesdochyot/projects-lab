...
5. [Розробка](#розробка)
6. [Документування](#документування)
7. [Висновки](#висновки)
...
## Документування Back-end

Документація бекенду була здійснена за допомогою JSDoc. JSDoc - це генератор документації для JavaScript, подібний до Javadoc або phpDocumentor. Ви додаєте коментарі до свого вихідного коду безпосередньо поряд із ним. Інструмент JSDoc просканує ваш вихідний код та створить для вас веб-сайт з документацією в форматі HTML.

### Додавання документації до вашого вихідного коду

Метою JSDoc є документування вашого додатка або бібліотеки на JavaScript. Передбачається, що ви захочете задокументувати такі речі як модулі, класи, методи, параметри методів тощо. Коментарі JSDoc зазвичай слід розміщувати безпосередньо перед документованим кодом. Кожен коментар повинен починатися з послідовності /\*\*, щоб бути розпізнаним парсером JSDoc.

![](https://raw.githubusercontent.com/Zvesdochyot/projects-lab/Nakarlovich/docs/screenshots/screenshots/BackendDocumentation.png)

## Документування API

Документація API була здійснена за допомогою Swagger. Swagger - це фреймворк для специфікації REST API. Його краса полягає в тому, що він дає можливість не тільки інтерактивно переглядати специфікацію, а й надсилати запити за допомогою так званого Swagger UI.

### Основні підходи

Swagger має два підходи до написання документації. Нижче наведено детальний опис кожного з них.

Якщо пишемо документацію на основі вашого коду:
- Даний підхід позиціонується як "дуже просто". Нам досить додати кілька залежностей в проект, додати конфігурацію і ми вже матимемо потрібну документацію, хоч і не настільки деталізовану;
- Код проекту стає не дуже читабельним від великої кількості анотацій та коментарів;
- Вся документація буде вписана в наш код.

Якщо пишемо документацію окремо від коду:
- Даний підхід вимагає знання синтаксису Swagger Specification.
- Документація пишеться в YAML / JSON файлі, або в редакторі Swagger Editor.

![](https://raw.githubusercontent.com/Zvesdochyot/projects-lab/Nakarlovich/docs/screenshots/screenshots/SwaggerDocumentation.png)
